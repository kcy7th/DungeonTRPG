...
namespace DungeonTRPG.Manager.Data
{
    internal class ActiveItemDB
    {
        ...
        public Dictionary<int, ActiveItem> Items { get; } = new Dictionary<int, ActiveItem>()
        {
            ...
            { 1010, new ActiveItem("돌맹이", "적에게 던져 데미지를 입힌다.", new List <Job> { Job.None }, new List<IEffect>(){ new HpDown(10) }, UseableIn.OnlyCombat) },
            { 1011, new ActiveItem("단검", "적에게 던져 데미지를 입힌다.", new List <Job> { Job.None }, new List<IEffect>(){ new HpDown(30) }, UseableIn.OnlyCombat) },
