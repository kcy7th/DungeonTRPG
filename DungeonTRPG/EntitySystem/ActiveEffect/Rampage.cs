...
namespace DungeonTRPG.EntitySystem.ActiveEffect
{
    internal class Rampage:IEffect
    {
        ...
        {
            if ((targets[0].Stat.Hp - amount * (1 + 0.2 * KillCount)) + targets[0].Stat.Def < 0) KillCount++; //타겟이 이 공격을 받은 뒤의 체력이 0보다 낮을 경우

            return targets[0].Damaged((int)(amount * (1 + 0.2 * KillCount)));
        }
    }
}
